...
import {
  ...
  ShieldAlert, Layers, ChevronDown, LayoutGrid, List, Check, Building2
} from 'lucide-react';
...
export default function VisaoGeralDiretorPage() {
  ...
  }, [projetosListFiltrados, config, fases, diarioLogs]);
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {selectedLoja !== 'TODAS' && (
                <span className="px-2.5 py-0.5 rounded-md text-[10px] font-bold bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
                  📍 Filial: {selectedLoja} ({projetosProcessados.length} {projetosProcessados.length === 1 ? 'obra' : 'obras'})
                </span>
              )}
            </div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
              {(session?.user as any)?.role === 'Gerente'
                ...
                ? selectedLoja !== 'TODAS' 
                  ? `Painel filtrado exclusivamente para a filial ${selectedLoja}. Mostrando apenas as obras desta localidade.`
